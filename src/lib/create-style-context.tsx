/**
 * @fileoverview Copied from Park UI:
 * https://park-ui.com/docs/panda/overview/getting-started
 */

/* eslint-disable react/display-name,@typescript-eslint/no-unsafe-argument,@typescript-eslint/no-unsafe-member-access */
import {
	createContext,
	createElement,
	forwardRef,
...
interface StyleRecipe {
	...
	splitVariantProps: (props: GenericProps) => unknown
}
...
const cx = (...args: Array<string | undefined>) =>
...
export const createStyleContext = <R extends StyleRecipe>(recipe: R) => {
	...
	const withProvider = <T extends ElementType>(
		...
		const StyledComponent = forwardRef((props: ComponentProps<T>, ref) => {
			// @ts-expect-error Copied from Park UI docs
